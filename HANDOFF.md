...
디자인·데모·본문을 한 번 더 손봤다. 요약:

- **디자인 방향 실구현** — 스펙의 "프로파일러의 측정 시트"가 CSS 40줄짜리 골격만 있던 상태였다. 측정지 눈금 배경, 계기판 형태의 히어로 지표, bento 위계(데모 카드 raised 표면), 세그먼티드 테마 스위치를 구현했다.
- **한국어 타이포 교정** — `max-width: 11ch` 때문에 "증명합니 / 다."처럼 어절이 잘렸다. `word-break: keep-all` + `text-wrap: balance`를 전역 기본값으로 두고 `ch` 기반 폭 제한을 제거했다.
- **히어로 지표가 살아났다** — `useReportWebVitals`는 LCP·CLS를 페이지가 숨겨질 때 확정 보고하므로 방문자가 보는 동안엔 영원히 "측정 중"이었다. `observeBufferedVitals`(PerformanceObserver `buffered: true`)로 로드 직후 실측값을 표시한다.
- **홈 번들 25KB 축소** — `Reveal`이 `LazyMotion`을 쓰면서 `domAnimation`을 정적 import해 framer-motion 전체가 홈 초기 번들에 들어 있었다. CSS 전환으로 교체했다. framer-motion은 이제 데모 청크 전용이다.
- **Stackflow 데모 제거** — 포지셔닝과 무관해 초점을 흐린다는 판단. 데모는 freeze·traffic 둘이다.
- **데모 UI 마감** — 컨트롤이 브라우저 기본값이고 차트에 축·제목이 없었다. 세그먼티드 컨트롤, 지표명·현재값·시간축, 실측(초록)과 시뮬레이션(주황) 구획 분리를 넣었다.
- **케이스스터디 본문 확장** — 결과만 있고 판단이 없었다. 제약 조건, 대안 검토, 안전요건, 측정 환경을 이력서 수위로 채웠다.
- **접근성 회귀 2건 수정** — 계기판 `nowrap`(320px 17px 초과), 테마 토글 3버튼 가로 배치(200% 확대 150px 초과). 둘 다 WCAG 1.4.10 위반이었고 게이트가 잡았다.
- **테스트 안정화** — freeze 데모가 메인스레드를 6초 점유하는데 워커 5개가 동시에 돌려 로드가 5초 대기를 넘겼다. 워커 3개, expect 15초로 조정(판정 기준은 불변).

## 본인 확인이 필요한 지점

본문의 다음 서술은 이력서에 없는 해석이라 실제 판단과 다르면 고쳐야 한다. 면접에서 그대로 나올 말이다.

1. `epub-comic-viewer.mdx` — "메시지를 주고받는 비용이 추가되지만, 그 비용은 페이지 넘김이 멈추는 것보다 감수할 만했습니다." Web Worker의 일반적 트레이드오프로는 타당하나 당시 실제 판단과 표현이 다를 수 있다.
2. `ai-workflow.mdx` — 밀리 바이브 배포 파이프라인은 단독, 팀 표준 6개 항목은 동료와 공동으로 구분했다. 실제 기여 범위와 맞는지 확인이 필요하다.
3. `isr-redis-cachehandler-poc.mdx` — 판단 매트릭스를 PoC 실측 제약(쿠키 의존 캐시 적중률, Pages Router 제어 범위) 중심으로 다시 짰다.
4. 각 사례 성과 섹션 마지막의 한 문장 회고(예: "안전요건을 먼저 정의한 것이 두 번째 원복을 막았습니다")는 구성된 해석이다.
